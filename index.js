...
const renderPokemons = pokemons => {
    const ul = document.querySelector('[data-js="lista-card"]');
    ...
    pokemons.forEach( ({ id, name, types, imgUrl }) => {
        ...
        h2.textContent = `${id}. ${name[0].toUpperCase()}${name.slice(1)}`
        p.textContent = types.length > 1 ? types.join(' | ') : firstType
        li.append(img, h2, p)
        fragment.append(li);
        ul.append(fragment);
    });
...
const observerLastPokemons = pokemonsObserver => {
    const lastPokemon = document.querySelector('[data-js="lista-card"]').lastChild
    pokemonsObserver.observe(lastPokemon)
}

const handleNextPokemonsRander = async () => {
    ...
    const pokemonsObserver = new IntersectionObserver(async ([lastPokemon], observe) => {
        ...
        const pokemons = await getPokemons("https://pokeapi.co/api/v2/pokemon?limit=15&offset=15")
        renderPokemons(pokemons)
        observerLastPokemons(pokemonsObserver)
    });
    observerLastPokemons(pokemonsObserver)
}

const handPageLoaded = async () => {
    const pokemons = await getPokemons();
    
    renderPokemons(pokemons)
    handleNextPokemonsRander()
}
